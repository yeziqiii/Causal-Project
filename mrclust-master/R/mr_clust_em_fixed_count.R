...
#' @export
mr_clust_em_fixed_count <- function(theta, theta_se, bx, by, bxse, byse,
                        obs_names = NULL, num_substantive_clusters = NULL,
  ...
  #cluster_sizes <- 0:m
  if (is.null(num_substantive_clusters)) {
    
    cluster_sizes <- 0:m
    
  } else {
    
    cluster_sizes <- num_substantive_clusters
...
attach(asNamespace("mrclust"))
res_em = mr_clust_em(theta = ratio_est, theta_se = ratio_est_se, bx = bx, by = by, bxse = bxse, byse = byse, obs_names = snp_names)
df <- data.frame(
  cluster_class = res_em$results$best$cluster_class,
  cluster_mean  = res_em$results$best$cluster_mean
)


aggregate(cluster_mean ~ cluster_class, data = df, FUN = mean)
